...
- Radii are moderate: 4px for anything that takes a value (fields, selects, checkboxes), 6px buttons, 8px panels, 12px a frame holding panels. Control heights are 28 / 32 / 36px, and 32 is the standard. Space is named by value — `--di-space-2 4 6 8 12 16 24 32 48` — so `--di-space-12` is 12px: fine at the bottom where the density is, coarse above 16. `Foundations/Scale` draws all three ramps.
